...
def configdataset(dataset_name, dir_main):
    """
    Load configuration for a given dataset
    
    Args:
        dataset_name: Name of the dataset (e.g., 'roxford5k', 'rparis6k', or custom)
        dir_main: Directory containing dataset folders
    
    Returns:
        cfg: Configuration dictionary
    """
    dataset_dir = os.path.join(dir_main, dataset_name)
    if not os.path.exists(dataset_dir):
        raise ValueError(f"Dataset directory not found: {dataset_dir}")
        
    gnd_file = os.path.join(dataset_dir, f'gnd_{dataset_name}.pkl')
    im_dir = os.path.join(dataset_dir, 'jpg')
    qim_dir = os.path.join(dataset_dir, 'jpg')
