...
def cann_search(q_feats, db_feats, k_candidates=1600, dim=128, backend="auto"):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [
            ...
            "--num_features", "500",      # BÌNH THƯỜNG LÀ 1000. Giảm xuống 500 hoặc 300 (Giảm số lượng điểm đặc trưng tính toán)
            "--num_grids", "3",      # BÌNH THƯỜNG LÀ 10. Giảm xuống 3 hoặc 5 (Giảm số lượng lưới tìm kiếm xấp xỉ)
            "--c", "8.0",          # BÌNH THƯỜNG LÀ 4.0. Tăng lên 8.0 (Nới lỏng độ xấp xỉ)
            "--dim", str(dim)
        ]
        
        print(f"Running CANN search with {len(q_feats)} queries and {len(db_feats)} database images...")
        try:
            # Bỏ capture_output để terminal hiển thị log theo thời gian thực
            subprocess.run(cmd, check=True)
        except subprocess.CalledProcessError as e:
            print(f"CANN execution failed with exit code: {e.returncode}")
            raise
